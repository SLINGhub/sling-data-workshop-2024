...
We can look at the internal standards in all samples across all six batches to see how the analyses went. The same ISTD amount was spiked into each sample (except `SBLK`) so we should expect the same intensities in all samples and sample types.
...
To proceed with further processing, we require additional metadata describing the samples and features. The `MiDAR Excel template` provides a solution for the collection, organisation and pre-validation of analysis metadata. Import metadata from this template using the function below. If there are errors in the metadata (e.g. duplicate or missing ID), the import will fail with an error message and summary of the errors. If the metadata is error-free, a summary of warnings and comments about the metadata will be shown in a table, if present. Check your metadata by working through these warnings, or proceed using `ignore_warnings = TRUE`.

> Open the `XLSM` file in the `data` subfolder to explore the metadata structure (click 'Disable Macros').
...
To examine overall technical trends and issues affecting the majority of analytes (features), the RLA (Relative Log Abundance) plot is a useful tool (De Livera et al., Analytical Chemistry, 2015). In this plot, all features are normalised (by across or within-batch medians) and plotted as a boxplot per sample. This plot can identify potential pipetting errors, sample spillage, injection volume changes or overall instrument sensitivity changes.

> First, run the code below as it is. What observations can be made? Then, examine batch 6, uncommenting the line `#analysis_no_range =`. What insights can be gained from this? Identify the potential outlier sample by setting `x_axis_variable = "analysis_id"`. Set the y-axis limits manually `y_lim = c(-2,2)` and display all analyses to ascertain whether there are other trends or fluctuations.
...
Based on the above RLA and PCA plots, we flagged a technical outlier and decided to remove it from all downstream processing `data_exclude_analyses()`.

> What do we now observe in the new PCA plot? Explore also different PCA dimensions (`pca_dim`).
...
A linear response in quantification is a prerequisite for the accurate measurement of differences in analyte concentrations between samples. Given the considerable dynamic range of plasma lipid species abundances and the fact that the class-specific ISTD is spiked at a single concentration, verifying the linear response can be a valuable aspect of the analytical quality. While optimising the injected sample amount is primarily a matter of quality assurance, differences in instrument performance can affect the dynamic range. Therefore, we measured injection volume series at the start and end of this analysis.

> Look at these response curves. What do we learn from these results? Change the plotted lipid species by modifying `feature_incl_filt` (it can use regular expressions). Save a PDF of all lipids by setting `save_pdf = TRUE` and commenting out (add a `#` in front of) `feature_incl_filt`
...
As demonstrated in the course presentation, there are several instances where the peaks of interest were co-integrated with the interfering isotope peaks of other lipid species. These intereferences can be subtracted from the raw intensities (areas) using the below function, which utilises information from the metadata.

> Check the sheet "Features (Analytes)" in the metadata file (folder `data`). Which species were affected? Why should we correct for M+3 isotope interference?
...
The first step is to normalise each lipid species with its corresponding internal standard (ISTD). Subsequently, the concentrations are calculated based on the volume of the spiked-in ISTD solution, the concentration of the ISTDs in this solution, and the sample volume.

> Visit the metadata template to view the corresponding details. You can also try to re-run e.g. above RLA and PCA plots using `variable = "norm_intensity"` or `variable = "conc".`
...
The use of class-specific ISTDs is common practice in lipidomics. However, non-authentic internal standards may elute at different times, which can result in them being subject to different matrix effects and thus different responses than the analytes. They may also have different fragmentation properties. Consequently, the use of non-authentic ISTDs for normalization can lead to the introduction of artefacts, Such artefacts can manifest as a consideranle increase in variability than the expected reduction in technical variability. It it may therefore be helpful to assess ISTDs in particular during QA, but also QC assements and to use an alternative ISTD when observing artifecatcs. One approach to detecting potential ISTD-related artefacts is to compare the variability of QC and samples before and after normalisation.

> What would you expect from such comarisons of CV? Do you notice potential issues with any of the ISTDs? What could be possible explanations for such an effect? And what would you do in this situation?
...
We're going to use a Gaussian kernel smoothing based on the study sample to correct for any drifts in the concentration data within each batch. This summary isn't meant to reflect the actual diagnostics of the fit, but rather to understand if the fit caused any artefacts. There is also an option to scale along the fit by setting `scale_smooth = TRUE`.
...
> Let's use this before defined function to plot the trends of one selected example before and after within-batch smoothing. What do we observe? Try changing `batch_wise = FALSE` in the code chunk above with `correct_drift_gaussiankernel()` to run the run the smoothing across all batches. Would this be a valid alternative? NOTE: don't forget to change back to `batch_wise = TRUE` after the test.
...
> Change the sample type to `qc_type = "BQC"` to use the BQC to center the batches. What do we see from this?
...
For additional inspection and documentation, we can save plots for all or a selected subset of species. It is often preferable to exclude blanks, as they can exhibit random concentrations when signals of features and internal standards are in close proximity or below the limit of detection. The corresponding PDF can be accessed within the `output` subfolder.

Try explore the effect of setting `cap_outliers` to `TRUE`or `FALSE`. Run `?runscatter` in the console or press `F2` on the function name to see all available options for `qc_plot_runscatter()`.
...
Finally, we apply a set of filters to exclude features that don't meet the specific QC criteria. The function `qc_apply_feature_filter()` lets you set the filters based on the data and metadata. Just press `TAB` after the open bracket of `qc_apply_feature_filter()` to select, or press `F1` or run `?qc_apply_feature_filter` to view, the available filter criteria. The filter function may be applied multiple times, either overwriting and amending previously set filters.

> Explore the effects of the different filtering criteria and filtering thresholds. The plot below can be run in order to examine the effects visually.
...
> Are there any differences between lipid classes in terms of their analytical performance? What are the identified QC issues and what are possible explainations for these? What could be the implications if you want to run the next analysis?
...
A record of the data post-processing can be generated in the form of an formatted `Excel` workbook comprising multiple sheets, each containing raw and processed datasets, associated metadata and feature quality control metrics.
...
You can also save specific data subsets as a clean flat, wide CSV file. This is how we shared the data for the statistical analysis that will be presented in the next part of the workshop.

> Specify which data to export using function arguments.
...
The myexp object can be saved as an `RDS` file and shared. `RDS` files are serialized R variables/objects that can be opened in R even in the absence of `midar`. The imported `MidarExperiment` object can be utilized for re-processing, plotting, or inspection using the `midar` package.

> Save the dataset to the disk and re-opened under a different name to ascertain its status. Compare it with the original dataset that is in the memory (`mexp`)
